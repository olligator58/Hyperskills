...
class ChainOfResponsibilityDemo {
     ...
     */
    static RequestHandler commonRequestHandler = wrapInTransactionTag.setSuccessor(createDigest.setSuccessor(wrapInRequestTag));
    ...
    interface RequestHandler {
        ...
        default RequestHandler setSuccessor(RequestHandler next) {
            ...
            return req -> next.handle(handle(req));

            /*return new RequestHandler() {
                @Override
                public Request handle(Request request) {
                    return next.handle(RequestHandler.this.handle(request));
                }
            };*/
        }
    }
